...
Config.Locations = {
	...
	{	job = "mechanic",
		...
		clockin = { { coords = vector3(-206.13, -1327.6, 30.89), heading = 184.43, }, },
...
RegisterNetEvent('jim-mechanic:client:Safe', function(data)
    if data.job == "mechanic" then
        TriggerEvent("inventory:client:SetCurrentStash", data.job .. "Safe")
        TriggerServerEvent("inventory:server:OpenInventory", "stash", data.job .. "Safe", {
            maxweight = 4000000,
            slots = 50,
        })
    else
        print("^1Access Denied: Only mechanics can open this stash!^7")
    end
end)
